...
streamdefcalc <- function(){
  ...
  # construct the four frames, top is exported by initializeDialog()
  optionsFrame <- tkframe(top)
  levelFrame <- tkframe(top)
  breakFrame <- tkframe(top)
  paraFrame <- tkframe(top)

  tablename <- tclVar(gettextRcmdr(paste0("StrdefTableof", ActiveDataSet())))
  ...
  radioButtons(window = optionsFrame,
               name = "pooling",
  ...
  radioButtons(window = levelFrame,
               name = "thresbreaks",
  ...
  entryminvol <- ttkentry(paraFrame, width = "15", textvariable = minvol)
  ...
  onOK <- function(){
    ...
    command <- paste("lfstat:::streamdefRcmdr(lfobj = ", ActiveDataSet(), ", pooling = \"",pool,"\", threslevel =",
